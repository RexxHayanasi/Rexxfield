--[[
	...
	Rexxfield Interface Suite

]]
-- Executor check
local executor = identifyexecutor and identifyexecutor() or (syn and "Synapse X") or (is_sirhurt_closure and "Sirhurt") or (KRNL_LOADED and "KRNL") or "Unknown"
print("Rexxfield loaded on executor:", executor)

if debugX then
	warn('Initialising Rexxfield')
end
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Rexxfield_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local RexxfieldFolder = "Rexxfield"
local ConfigurationFolder = RexxfieldFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		RexxfieldOpen = {Type = 'bind', Value = 'K', Name = 'Rexxfield Keybind'},
		-- buildwarnings
		-- Rexxfieldprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.RexxfieldOpen"] = "J"
...
--local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/RexxHayanasi/SeriusID/refs/heads/request/prompt.lua')
local request = (syn and syn.request) or (fluxus and fluxus.request) or (http and http.request) or http_request or (request and typeof(request) == "function" and request) or function() error("Executor does not support 'request'") end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(RexxfieldFolder) then
				if isfile and isfile(RexxfieldFolder..'/settings'..ConfigurationExtension) then
					file = readfile(RexxfieldFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"RexxfieldOpen":{"Value":"K","Type":"bind","Name":"Rexxfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Rexxfield Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Rexxfield had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Rexxfield", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Rexxfield developer.")
				end
			end)
...
local Rexxfield = useStudio and script.Parent:FindFirstChild('Rexxfield') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local RexxfieldDestroyed = false -- True when RexxfieldLibrary:Destroy() is called

repeat
	if Rexxfield:FindFirstChild('Build') and Rexxfield.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Rexxfield | Build Mismatch')
		print('Rexxfield may encounter issues as you are running an incompatible interface version ('.. ((Rexxfield:FindFirstChild('Build') and Rexxfield.Build.Value) or 'No Build') ..').\n\nThis version of Rexxfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Rexxfield = Rexxfield, useStudio and script.Parent:FindFirstChild('Rexxfield') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Rexxfield.Enabled = false

elseif not gethui and typeof(get_hidden_gui) == "function" then
	Rexxfield.Parent = get_hidden_gui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Rexxfield)
	Rexxfield.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Rexxfield.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Rexxfield.Name and Interface ~= Rexxfield then
			Interface.Enabled = false
			Interface.Name = "Rexxfield-Old"
		end
	end
elseif not useStudio then
...
if Rexxfield.AbsoluteSize.X < minSize.X and Rexxfield.AbsoluteSize.Y < minSize.Y then
...
local Main = Rexxfield.Main
local MPrompt = Rexxfield:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Rexxfield:FindFirstChild('Drag')
...
Rexxfield.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/RexxHayanasi/Rexxfield/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Rexxfield.Notifications

local SelectedTheme = RexxfieldLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RexxfieldLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Rexxfield.Main.BackgroundColor3 = SelectedTheme.Background
	Rexxfield.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Rexxfield.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Rexxfield.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Rexxfield.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Rexxfield.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Rexxfield.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Rexxfield.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Rexxfield.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Rexxfield:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Rexxfield | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Rexxfield | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Rexxfield had an issue decoding the configuration file, please try delete the file and reopen Rexxfield.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(RexxfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Rexxfield | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RexxfieldLibrary:Notify({Title = "Rexxfield Flags", Content = "Rexxfield was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(RexxfieldLibrary.Flags) do
...
function RexxfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Rexxfield | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RexxfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Rexxfield'.", Duration = 7, Image = 4400697855})
		else
			RexxfieldLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "RexxfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(RexxfieldFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Rexxfield Settings', 0, true)

	if TabList['Rexxfield Settings'] then
		TabList['Rexxfield Settings'].LayoutOrder = 1000
	end

	if Elements['Rexxfield Settings'] then
		Elements['Rexxfield Settings'].LayoutOrder = 1000
	end
...
function RexxfieldLibrary:CreateWindow(Settings)
	if Rexxfield:FindFirstChild('Loading') then
		if getgenv and not getgenv().RexxfieldCached then
			Rexxfield.Enabled = true
			Rexxfield.Loading.Visible = true

			task.wait(1.4)
			Rexxfield.Loading.Visible = false
		end
	end

	if getgenv then getgenv().RexxfieldCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RexxfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'Rexxfield may encounter issues as you are running an incompatible interface version ('.. ((Rexxfield:FindFirstChild('Build') and Rexxfield.Build.Value) or 'No Build') ..').\n\nThis version of Rexxfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "RexxfieldOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "RexxfieldOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(RexxfieldFolder) then
		makefolder(RexxfieldFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Rexxfield"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Rexxfield Interface Suite" then
		LoadingFrame.Version.Text = "Rexxfield UI"
	end
	...
	if not Settings.DisableRexxfieldPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				RexxfieldLibrary:Notify({
					Title = "Rexxfield Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(RexxfieldFolder.."/Discord Invites") then
			makefolder(RexxfieldFolder.."/Discord Invites")
		end

		if isfile and not isfile(RexxfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RexxfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Rexxfield RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(RexxfieldFolder.."/Key System") then
			makefolder(RexxfieldFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Rexxfield | "..Key.." Error " ..tostring(Response))
					warn('Check for help with Rexxfield specific development.')
				end
			end
		end
		...
		if isfile and isfile(RexxfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(RexxfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RexxfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						RexxfieldLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called RexxfieldLibrary:Destroy()
				if RexxfieldDestroyed then
					return
				end
				if not Success then
					...
					print("Rexxfield | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rexxfield specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= RexxfieldLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					RexxfieldLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Rexxfield.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Rexxfield | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rexxfield specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					RexxfieldLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Rexxfield | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rexxfield specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					RexxfieldLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Rexxfield | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rexxfield specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					RexxfieldLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= RexxfieldLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Rexxfield | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rexxfield specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						RexxfieldLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Rexxfield | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Rexxfield specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					RexxfieldLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RexxfieldLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			RexxfieldLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Rexxfield had an issue creating settings.') end
...
function RexxfieldLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function RexxfieldLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RexxfieldLibrary:Destroy()
	RexxfieldDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Rexxfield Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "RexxfieldOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function RexxfieldLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RexxfieldLibrary:Notify({Title = "Rexxfield Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RexxfieldLibrary:Notify({Title = "Rexxfield Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Rexxfield Configurations Error | '..tostring(result))
			RexxfieldLibrary:Notify({Title = "Rexxfield Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = RexxfieldLibrary:CreateWindow({
		Name = "Rexxfield Example Window",
		LoadingTitle = "Rexxfield Interface Suite",
		Theme = 'Default',
		Icon = 0,
	...
	--RexxfieldLibrary:Notify({Title = "Rexxfield Interface", Content = "Welcome to Rexxfield. These - are the brand new notification design for Rexxfield, with custom sizing and Rexxfield calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(RexxfieldLibrary.Theme) do
...
-- 	task.spawn(loadWithTimeout, "https://raw.githubusercontent.com/RexxHayanasi/SeriusID/refs/heads/request/prompt.lua")
-- end

task.delay(4, function()
	RexxfieldLibrary.LoadConfiguration()
